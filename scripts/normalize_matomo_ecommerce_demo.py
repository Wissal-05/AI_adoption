...
from adoption_analytics.data_sources.matomo import load_latest_matomo_usage_events
...
def main() -> None:
    usage_df = load_latest_matomo_usage_events(
        RAW_DIR,
    ...
    print(f"Fichier normalisé exporté : {output_path}")
    print(f"Lignes générées : {len(usage_df)}")

    if usage_df.empty:
        print("Aucune donnée normalisée.")
        return

    print(f"Services : {usage_df['service'].unique().tolist()}")
    print(f"Utilisateurs synthétiques : {usage_df['user_id'].nunique()}")
    print(f"Sessions synthétiques : {usage_df['session_id'].nunique()}")
