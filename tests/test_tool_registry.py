...
def test_all_tools_schema_present(registry):
    tools = registry.list_tools()
    assert len(tools) == 5
    names = [t["name"] for t in tools]
    assert "get_adoption_by_module" in names
    assert "get_adoption_by_campus" in names
    assert "get_top_interactions" in names
...
def test_integration_real_data_all_tools():
    service = DashboardService()
    try:
        service.load()
    except Exception:
